...
import { Search, ArrowUpRight, Briefcase } from "lucide-react";
...
const STATUS_COLORS: Record<string, string> = {
  in_preparation: "bg-slate-100 text-slate-800",
...
export function MyProjectsPage() {
  ...
  const states = [...new Set(RO_PROJECTS.flatMap((p) => p.states))].sort();
  const stages = [...new Set(RO_PROJECTS.map((p) => p.currentStage))].sort();

  const filtered = RO_PROJECTS.filter((p) => {
    ...
    const matchState = stateFilter === "all" || p.states.includes(stateFilter);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold tracking-tight text-[#0F2340]">My Projects</h1>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5">
            Organisation project portfolio — NHAI
          </p>
        </div>
        <Button asChild size="sm">
          <Link to="/app/ro/create">Create Project</Link>
        </Button>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-slate-50">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-slate-700">State(s)</th>
              ...
              <tbody>
                {filtered.map((project) => (
                  <tr key={project.id} className="border-b last:border-0 hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-3 text-slate-600 text-[11px]">{project.states.join(", ")}</td>
                    <td className="px-4 py-3 text-center">{project.parcels.toLocaleString()}</td>
                    ...
                    <td className="px-4 py-3">
                      <Badge className={`text-[11px] ${STATUS_COLORS[project.status]}`}>
                        {project.status.replace(/_/g, " ")}
                      </Badge>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <Button asChild variant="ghost" size="sm">
                        <Link to={`/app/ro/project/${project.id}`}>

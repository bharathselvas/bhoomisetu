...
import { MINISTRY_RISK_PROJECTS } from "@/features/ministry/ministryData";
...
export function MinistryRiskDelayPage() {
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Badge variant="secondary" className="text-[11px]">
          {MINISTRY_RISK_PROJECTS.length} at risk
        </Badge>
      </div>

      <div className="space-y-3">
        {MINISTRY_RISK_PROJECTS.map((project, idx) => (
          <Card key={idx} className={project.risk === "critical" ? "border-red-200" : "border-orange-200"}>
            <CardContent className="p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className={`text-[10px] ${RISK_COLORS[project.risk]}`}>
                  ...
                  <div className="mt-2 p-2 bg-red-50 rounded text-[11px] text-red-800">
                    {project.reason}
                  </div>
                ...
                <Button asChild variant="outline" size="sm" className="shrink-0">
                  <a href={`/app/ministry/project/${project.projectId}`}>

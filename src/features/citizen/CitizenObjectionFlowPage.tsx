import { useState } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { AlertTriangle, FileWarning, CheckCircle2 } from "lucide-react";
import { CITIZEN_PROJECTS } from "./citizenData";
import type { ObjectionCategory } from "./citizenData";

const CATEGORY_OPTIONS: { value: ObjectionCategory; label: string }[] = [
  { value: "land_ownership", label: "Land Ownership" },
  { value: "parcel_boundary", label: "Parcel Boundary" },
  { value: "land_area", label: "Land Area" },
  { value: "land_use", label: "Land Use" },
  { value: "livelihood_impact", label: "Livelihood Impact" },
  { value: "public_property", label: "Public Property" },
  { value: "residential_impact", label: "Residential Impact" },
  { value: "other", label: "Other" },
];

type Step = "form" | "review" | "submitted";

export default function CitizenObjectionFlowPage() {
  const [step, setStep] = useState<Step>("form");
  const [project, setProject] = useState("");
  const [parcel, setParcel] = useState("PAR-JLN-00421");
  const [name, setName] = useState("");
  const [contact, setContact] = useState("");
  const [category, setCategory] = useState("");
  const [description, setDescription] = useState("");
  const [consent, setConsent] = useState(false);

  const canSubmit = project && parcel && name && contact && category && description && consent;

  if (step === "submitted") {
    return (
      <div className="space-y-6">
        <div className="bg-amber-100 border border-amber-300 rounded-md px-4 py-2 text-center text-sm font-medium text-amber-800">
          MOCK / SANDBOX — All data is fictional and for demonstration only
        </div>
        <Card className="border-[#0F7A5A]">
          <CardContent className="pt-6 text-center space-y-4">
            <div className="mx-auto w-12 h-12 rounded-full bg-[#0F7A5A]/10 flex items-center justify-center">
              <CheckCircle2 className="h-6 w-6 text-[#0F7A5A]" />
            </div>
            <h2 className="text-lg font-bold text-[#0F2340]">Objection Submitted</h2>
            <p className="text-sm text-muted-foreground">Your objection has been successfully submitted.</p>
            <div className="max-w-sm mx-auto space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Objection ID</span>
                <span className="font-medium">OBJ-2026-00881</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Date</span>
                <span className="font-medium">10 Sep 2026</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Status</span>
                <Badge variant="info">Submitted</Badge>
              </div>
            </div>
          </CardContent>
        </Card>
      </div>
    );
  }

  return (
    <div className="space-y-6">
      <div className="bg-amber-100 border border-amber-300 rounded-md px-4 py-2 text-center text-sm font-medium text-amber-800">
        MOCK / SANDBOX — All data is fictional and for demonstration only
      </div>

      <div className="flex items-center gap-3">
        <div className="bg-[#0F2340] p-2 rounded-lg">
          <FileWarning className="h-5 w-5 text-white" />
        </div>
        <div>
          <h1 className="text-xl font-bold text-[#0F2340]">File an Objection</h1>
          <p className="text-sm text-muted-foreground">Exercise your right under the LARR Act, 2013</p>
        </div>
      </div>

      <Card className="border-amber-200 bg-amber-50">
        <CardContent className="pt-5">
          <div className="flex items-start gap-3">
            <AlertTriangle className="h-5 w-5 text-amber-600 mt-0.5" />
            <div className="text-sm text-amber-800 space-y-1">
              <p className="font-semibold">Your Right to Object</p>
              <p>
                Under Section 11(1) of the Right to Fair Compensation and Transparency in Land Acquisition,
                Rehabilitation and Resettlement Act, 2013 (LARR Act), any person whose interest in the land
                is affected may file an objection within 30 days of the preliminary notification.
              </p>
              <p>Objections can relate to ownership, boundaries, land area, land use, livelihood impact, or any other ground.</p>
            </div>
          </div>
        </CardContent>
      </Card>

      {step === "form" && (
        <Card>
          <CardHeader>
            <CardTitle className="text-[#0F2340]">Objection Form</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>Project</Label>
              <Select value={project} onValueChange={setProject}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a project" />
                </SelectTrigger>
                <SelectContent>
                  {CITIZEN_PROJECTS.map((p) => (
                    <SelectItem key={p.projectId} value={p.projectId}>{p.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>

            <div className="space-y-2">
              <Label>Parcel Reference</Label>
              <Input value={parcel} onChange={(e) => setParcel(e.target.value)} />
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label>Full Name</Label>
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your full name" />
              </div>
              <div className="space-y-2">
                <Label>Contact Number</Label>
                <Input value={contact} onChange={(e) => setContact(e.target.value)} placeholder="Mobile number" />
              </div>
            </div>

            <div className="space-y-2">
              <Label>Objection Category</Label>
              <Select value={category} onValueChange={setCategory}>
                <SelectTrigger>
                  <SelectValue placeholder="Select category" />
                </SelectTrigger>
                <SelectContent>
                  {CATEGORY_OPTIONS.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>

            <div className="space-y-2">
              <Label>Reason / Description</Label>
              <textarea
                className="flex min-h-[120px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Describe your objection in detail..."
              />
            </div>

            <div className="space-y-2">
              <Label>Supporting Document</Label>
              <div className="border-2 border-dashed rounded-md p-4 text-center text-sm text-muted-foreground">
                <p>Drag and drop a file here, or click to browse</p>
                <p className="text-xs mt-1">PDF, JPG, PNG (max 5MB) — Mock upload</p>
              </div>
            </div>

            <div className="flex items-start gap-2">
              <input
                type="checkbox"
                id="consent"
                checked={consent}
                onChange={(e) => setConsent(e.target.checked)}
                className="mt-1"
              />
              <label htmlFor="consent" className="text-sm text-muted-foreground">
                I confirm that the information provided is true and correct to the best of my knowledge.
              </label>
            </div>

            <div className="flex justify-end">
              <Button onClick={() => setStep("review")} disabled={!canSubmit}>
                Review Objection
              </Button>
            </div>
          </CardContent>
        </Card>
      )}

      {step === "review" && (
        <Card>
          <CardHeader>
            <CardTitle className="text-[#0F2340]">Review Objection</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Project</span>
              <span className="font-medium">{CITIZEN_PROJECTS.find((p) => p.projectId === project)?.name}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Parcel</span>
              <span className="font-medium">{parcel}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Name</span>
              <span className="font-medium">{name}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Contact</span>
              <span className="font-medium">{contact}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Category</span>
              <span className="font-medium">{CATEGORY_OPTIONS.find((c) => c.value === category)?.label}</span>
            </div>
            <div className="space-y-1 text-sm">
              <span className="text-muted-foreground">Description</span>
              <p className="text-slate-700">{description}</p>
            </div>
            <div className="flex justify-between gap-2 pt-4">
              <Button variant="outline" onClick={() => setStep("form")}>Back to Edit</Button>
              <Button onClick={() => setStep("submitted")} className="bg-[#0F7A5A] hover:bg-[#0d6a4d] text-white">Submit</Button>
            </div>
          </CardContent>
        </Card>
      )}
    </div>
  );
}
